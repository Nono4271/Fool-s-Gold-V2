...
import { SC, SS } from "../../constants/heroes.js";
                  ...
                  <div key={cmd.uid} style={{background:"rgba(255,255,255,.03)",border:`1px solid ${SC(cmd.star)}40`,borderRadius:6,padding:"10px 12px",display:"flex",gap:10,alignItems:"flex-start",flexWrap:"wrap"}}>
                    ...
                    <div style={{flex:1,minWidth:130}}>
                      ...
                      {(cmd.lvl||5)<CMD_LVL_MAX && (() => {
                        ...
                        return (
                          <div style={{marginBottom:4}}>
                            <div style={{display:"flex",justifyContent:"space-between",fontSize:7,color:"#5a5a6a",marginBottom:2,fontFamily:"'Cinzel',serif"}}><span>XP</span><span>{cmd.xp||0}/{needed}</span></div>
                      ...
                      <div style={{marginBottom:4,fontSize:8,color:"#7a8a9a",fontFamily:"'Cinzel',serif"}}>
                        📡 Command: <span style={{color:"#a0c0e0",fontWeight:700}}>{cmdCommand(cmd.lvl||5,bldgs.commandcenter||0).toLocaleString()}</span>
                      ...
                      <div style={{fontSize:9,color:"#7a6a5a",lineHeight:1.7,fontFamily:"'Crimson Pro',serif",marginBottom:4}}>
                        ...
                        {(() => {
                          ...
                          return <span style={{color:isWiz?(isMend?"#40e898":"#bb66ff"):"#8030cc"}}>{isWiz?(isMend?"💚 ":"🔮 "):"⚔ "}{cmd.skill}</span>;
                        })()}<br/>
                        {(() => { const pf = PLAYABLE_FACTIONS.find(f=>f.key===cmd.faction); return pf?<span style={{color:pf.c,fontSize:8}}>{pf.s} {pf.n}</span>:null; })()}<br/>
                        {(() => {
                          const isHQTile = cmd.tk===hqKey;
                          if (cmd.march) {
                            const destTerrain = tiles[cmd.march.dest]?.terrain;
                            const destLabel = cmd.march.dest===hqKey?"HQ":(destTerrain?TERR[destTerrain]?.lbl||destTerrain:cmd.march.dest);
                        ...
                        {cmd.march && <button className="btn" onClick={()=>recallMarch(cmd.uid)} style={{marginLeft:6,padding:"1px 6px",background:"rgba(200,80,80,.2)",border:"1px solid #cc4444",color:"#ff8888",fontSize:7}}>↩ Recall</button>}
                      </div>
                    </div>
