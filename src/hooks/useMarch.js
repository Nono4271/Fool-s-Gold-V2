...
if (newXp >= needed) { newXp -= needed; newLvl++; leveled = true; } else break;
}
if (newLvl >= CMD_LVL_MAX) newXp = 0;
if (leveled && floaty) floaty(`⬆ Lv${newLvl}!`, “#f0c040”, cmd.tk);
return { xp: newXp, lvl: newLvl };
}

export function useMarch({
screen, tiles, bldgs, cmds,
setCmds, setTiles, setWounded, setBarracks, setBattles, setBLog, setWinner, setUnseenBattles,
tilesRef, floaty,
}) {
...
arrivedAttackers.forEach(cmd => {
  ...
  if (defTile.garrisonDefeated) {
    ...
    const siegePower = calcSiegePower(newTroops, cmd.troopType, require("../constants/troops.js").TROOP);
  ...
  const { TROOP } = require("../constants/troops.js");
  const siegePower   = calcSiegePower(finalTroops, cmd.troopType, TROOP);
  const currentSiege = defTile.siege ?? SIEGE_BASE;
...
arrivedAI.forEach(cmd => {
  ...
  if (defTile.garrisonDefeated) {
    const { TROOP } = require("../constants/troops.js");
    const siegePower = calcSiegePower(cmd.troops||0, cmd.troopType, TROOP);
    const currentSiege = defTile.siege ?? SIEGE_BASE;
    if (siegePower >= currentSiege) {
  ...
  if (res.won) {
    const { TROOP } = require("../constants/troops.js");
    const siegePower = calcSiegePower(newTroops, cmd.troopType, TROOP);
    const currentSiege = defTile.siege ?? SIEGE_BASE;
    if (siegePower >= currentSiege) {
      tileCaptured = true;
      const isPlayerHQ = defTile.isHQ && defTile.owner === "player";
