...
import { HDEFS, SC, SS, rollGacha } from "./constants/heroes.js";
...
export default function RiseToWar() {
  ...
  const [cmds,   setCmds]    = useState(() => HDEFS.filter(h => h.faction==="pirates").map((h,i) => ({...h, uid:`p${i}`, owner:"player", troops:0, troopType:null, tk:`${HQP.player.c},${HQP.player.r}`, lvl:5, xp:0})));
  ...
  const pull = useCallback(n => {
    ...
    setCmds(p => { const nx=[...p]; res.forEach(h => { if (!nx.find(x=>x.id===h.id)) nx.push({...h, uid:h.uid, troops:0, troopType:null, tk:hqk, owner:"player", lvl:5, xp:0}); }); return nx; });
  ...
  if (screen==="gacha")   return (
    <GachaScreen
      ...
      showG={showG} gRes={gRes} coll={coll}
  ...
  return (
    <div style={{width:"100vw",height:"100vh",position:"relative",overflow:"hidden",background:"#0e1014",userSelect:"none",touchAction:"none"}}>
      ...
      <HUD
        facName={facName} pKeys={pKeys} rss={rss} gems={gems}
        setHqOpen={setHqOpen} setHqTab={setHqTab} setSelKey={setSelKey}
        setMode={setMode} setScreen={setScreen}
      />
      ...
      <div style={{position:"fixed",right:10,top:46,zIndex:190,display:"flex",flexDirection:"column",gap:4}}>
        ...
        <button className="btn" onClick={() => setZoom(prev => ZOOM_LEVELS[Math.max(0,ZOOM_LEVELS.indexOf(prev)-1)])}
          style={{width:36,height:36,background:"rgba(8,10,14,.92)",border:"1px solid #2a2010",color:"#c8a060",fontSize:18,fontWeight:700,display:"flex",alignItems:"center",justifyContent:"center",borderRadius:6,boxShadow:"0 2px 8px rgba(0,0,0,.5)"}}>−</button>
        <div style={{height:6}}/>
        <button className="btn" onClick={() => { setShowBattleLog(true); setUnseenBattles(0); }} title="Battle Reports"
          ...
          ⚔{unseenBattles>0&&<div style={{position:"absolute",top:-4,right:-4,width:16,height:16,background:"#cc3030",borderRadius:"50%",fontSize:7,color:"#fff",display:"flex",alignItems:"center",justifyContent:"center",fontFamily:"'Cinzel',serif",fontWeight:700}}>{Math.min(99,unseenBattles)}</div>}
        </button>
      </div>
